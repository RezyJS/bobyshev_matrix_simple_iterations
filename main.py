...
if __name__ == "__main__":
    ...
    epsilon: int = int(
        input("Введите точность (без знака, количество чисел после запятой): ")
    )

    # Указываем размер матрицы (n x n)
    length: int = int(input("Введите размер матрицы: "))
    if length <= 0:
        raise ValueError("Размер матрицы не может быть меньше единицы.")
    print()

    # Считываем матрицу и вектор-столбец свободных членов
    coefficients_matrix: list[list[float]] = slau.read_matrix(length)
    free_terms_vector: list[float] = slau.read_vector(
        "Введите столбец свободных членов: ", length)

    coefficients_matrix_copy: list[list[float]] = [
        x.copy() for x in coefficients_matrix]
    ...
    slau.normalize_my_matrix(coefficients_matrix_copy,
                             free_terms_vector_copy, length)
    ...
    if 1 < alpha[1] or alpha[1] < 0 or 1 < beta[1] or beta[1] < 0 or 1 < gamma[1] or gamma[1] < 0:
        ...
        print('\nx1:')
        slau.print_matrix_for_x1(
            coefficients_matrix_copy, free_terms_vector_copy, epsilon)

        print(
            f"\nx1 вектор после преобразования: \n{(x1 := vf.sum_vectors(
                vf.multiply_matrix_vector(coefficients_matrix_copy, x0), free_terms_vector_copy))}\n"
        )

        iterations_k: int = 0
        iterations_k_list: list[int] = []

        iterations_k_list.append(slau.get_iterations_value(
            epsilon, metrics.ro_inf, x0, x1, alpha))
        iterations_k_list.append(slau.get_iterations_value(
            epsilon, metrics.ro_1, x0, x1, beta))
        iterations_k_list.append(slau.get_iterations_value(
            epsilon, metrics.ro_2, x0, x1, gamma))

        if (alpha[0] is True):
            iterations_k = iterations_k_list[0]
        elif (beta[0] is True):
            iterations_k = iterations_k_list[1]
        elif (gamma[0] is True):
            iterations_k = iterations_k_list[2]

        print("Число итераций для:")
        print(f"{'alpha:'.ljust(7)} {iterations_k_list[0]}")
        print(f"{'beta:'.ljust(7)} {iterations_k_list[1]}")
        print(f"{'gamma:'.ljust(7)} {iterations_k_list[2]}")
        print()
            ...
            for i in range(2, iterations_k + 1):
                x_n = vf.sum_vectors(
                    vf.multiply_matrix_vector(
                        coefficients_matrix_copy, x_list[i - 1]),
                    free_terms_vector_copy,
                )
                x_list.append(x_n)
